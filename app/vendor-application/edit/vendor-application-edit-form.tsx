...
import { STUB_PLATE, StubPerf } from "@/components/vendor-application/stub-plate"
...
interface VendorApplicationEditFormProps {
  application: {
    ...
    created_at: string
    updated_at: string
    /** When the decision being answered was made. Read by the rail on the resubmit path. */
...
export function VendorApplicationEditForm({
  ...
  return (
    <div className="mt-[clamp(2.5rem,5vw,3.5rem)] lg:grid lg:grid-cols-[2fr_3fr] lg:gap-16 xl:gap-20">
      ...
      <div>
        {/* The checkout stub plate /become-vendor holds the same fields on: its cap, then the
            tear. No transform on this wrapper or the plate, so the action row below still
            docks to the viewport rather than to the column on a phone. */}
        <div className={STUB_PLATE}>
          <div className="checkout-stub-cap">
            <h2 className="checkout-stub-ref">Application details</h2>
        ...
        {/* Clears the docked action, which is out of flow below lg. Outside the plate, as on
            /become-vendor, so the plate does not grow an empty band on a phone. */}
