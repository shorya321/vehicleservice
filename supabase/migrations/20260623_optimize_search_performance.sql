-- Optimize search_locations to eliminate statement timeouts (error 57014).
--
-- Root cause: the previous version evaluated similarity(), multiple EXISTS,
-- unnest + regex per row across the ENTIRE locations table. With thousands
-- of rows, this exceeded the statement timeout.
--
-- Fix: two-phase CTE approach.
-- Phase 1 (candidates): cheap ILIKE filtering using GIN trigram indexes, capped at 150 rows.
-- Phase 2 (scoring): full relevance scoring on max 150 candidates only.
-- Removed: similarity() everywhere, regex word-boundary matching (~* \m), duplicate EXISTS.

-- Index for fast exact alias lookups (DXB, DWC, etc.)
CREATE INDEX IF NOT EXISTS idx_location_aliases_lower_alias
...
BEGIN
  ...
  -- Phase 1: candidate filtering (GIN trigram indexes, capped at 150)
  WITH candidates AS (
    SELECT
  ...
  -- Phase 2: score candidates (max 150 rows)
  scored AS (
    SELECT
      c.id,
      c.name::TEXT,
