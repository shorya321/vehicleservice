-- Fix: alias-only matches (e.g. DXB → Dubai International Airport) were excluded
-- from candidates because 265+ locations with "dxb" in their name filled the
-- 150-row candidate cap before alias-only matches (ORDER BY case 4) got a slot.
--
-- Fix: promote alias exact matches to case 1 in both query branches.
